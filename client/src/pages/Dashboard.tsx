...
const Dashboard = () => {
  ...
  const totalActivityMinutes: number = todayActivities.reduce((sum, item)=>
  sum + item.duration, 0)
  ...
  return (
    <div className="page-container">
      ...
      <div className="dashboard-header">
        ...
        mt-1">{`Hi There! ${user?.username}`}</h1>
      ...
      <div className="dashboard-grid">
        ...
        <Card className="shadow-lg col-span-2">
          <div className="flex items-center justify-between mb-4">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                dark:text-white">{totalCalories}</p>
              </div>

            </div>
            <div className="text-right">
              <p className="text-sm text-slate-500
              dark:text-slate-400">Limit</p>
              <p className="text-2xl font-bold text-slate-800
              dark:text-white">{DAILY_CALORIE_LIMIT}</p>
            </div>
          </div>
          ...
          <div className="mt-4 flex justify-between items-center">
            ...
            ? 'bg-emerald-50 dark:bg-emerald-900/10 text-emerald-700 dark:text-emerald-400' 
              ...
              <span className="text-sm font-medium">{remainingCalories >= 0 
              ? `${remainingCalories} kcal remaining` :
                `${Math.abs(remainingCalories)} kcal over`}</span>
            </div>
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                <p className="text-sm text-slate-500 dark:text-slate-400">
                  Calories Burned</p>
                <p className="text-2xl font-bold text-slate-800
                dark:text-white">{totalBurned}</p>
              </div>

            </div>
            <div className="text-right">
              <p className="text-sm text-slate-500
              dark:text-slate-400">Goal</p>
              <p className="text-2xl font-bold text-slate-800
              dark:text-white">{user?.dailyCalorieBurn || 400}</p>
            </div>
        ...
        <div className="dashboard-card-grid">
          ...
          <Card>
            ...
            <p className="text-2xl font-bold text-slate-800
            dark:text-white">{totalActivityMinutes}</p>
            <p>Minutes today</p>
          </Card>
          {/* Activivities Count */}
          <Card>
            ...
            <p className="text-2xl font-bold text-slate-800
            dark:text-white">{todayActivities.length}</p>
        ...
        {user && user.weight && (
          <Card>
            <div className="flex items-center gap-4 mb-6">
              <div className="w-12 h-12 rounded-xl bg-indigo-100 flex items-center
              justify-center">
            ...
            <div className="space-y-4">
              <div className="flex justify-between items-center">
                ...
                dark:text-slate-200">{user.weight} kg</span>
              </div>
              ...
              {user.height && (
                ...
                <span className="font-semibold text-slate-700
                dark:text-slate-200">{user.height} cm</span>
              </div>
              )}
              ...
              {user.height && (
                ...
                dark:border-slate-800">
                  <div className="flex justify-between items-center mb-2">
                    ...
                    dark:text-slate-300">Body mass index</span>
                    {(()=>{
                      ...
                      return <span className={`text-lg font-bold 
                        ${status.color}`}>{bmi}</span>
                      })()}
                  </div>
                  {/* BMI Scale Visual  */}
                  <div className="h-2 w-full bg-slate-100
                  dark:bg-slate-800 rounded-full overflow-hidden flex">
        ...
        <Card>
          <h3 className="font-semibold text-slate-800 dark:text-white mb-4">Today summary</h3>
          ...
          <div className="space-y-3">
            ...
            border-slate-100 dark:border-slate-800">
              ...
              dark:text-slate-200">{todayFood.length}</span>
            </div>
            <div className="flex justify-between items-center py-2 border-b
            border-slate-100 dark:border-slate-800">
              <span className="text-slate-500 dark:text-slate-400
              ">Total Calories</span>
              <span className="font-medium text-slate-700
              dark:text-slate-200">{totalCalories} kcal</span>
            </div>
            <div className="flex justify-between items-center py-2 ">
              <span className="text-slate-500 dark:text-slate-400
              ">Active Times</span>
              <span className="font-medium text-slate-700
              dark:text-slate-200">{totalActivityMinutes} min</span>
            </div>
